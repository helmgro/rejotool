...
MultiRejoinTool.prototype.injectCookiesFromFile = async function(rl) {
  if (!fs.existsSync(COOKIES_FILE)) {
    console.log("❌ No existe cookies.txt en la carpeta del script.");
    return;
  }
  ...
  if (lines.length === 0) {
    console.log("❌ El archivo cookies.txt está vacío.");
    return;
  }

  // Detectar paquetes Roblox instalados
  const packages = Utils.detectAllRobloxPackages();
  const packageNames = Object.keys(packages);
  if (packageNames.length === 0) {
    console.log("❌ No se detectan apps Roblox instaladas.");
    return;
  }

  console.log("\n🚀 Inyectando cookies...");
  lines.forEach((line, i) => {
    // Formato esperado: user:pass:cookie
    const [user, pass, cookie] = line.split(":");
    const pkg = packageNames[i];
    if (!pkg || !cookie) {
      console.log(`❌ Línea ${i+1}: Sin paquete o cookie.`);
      return;
    }

    // Inyectar cookie sobreescribiendo el archivo Cookies
    try {
      // Generar contenido mínimo sólo con la cookie (puedes mejorar para tu formato)
      execSync(`echo '.ROBLOSECURITY=${cookie.trim()}' > /data/data/${pkg}/app_webview/Default/Cookies`);
      console.log(`✅ [${pkg}] Injectada cookie para usuario: ${user || "Desconocido"}`);
    } catch (e) {
      console.log(`❌ Error injectando en ${pkg}: ${e.message}`);
    }
  });

  // Mostrar resumen
  console.log("\n🍪 Resumen de cuentas inyectadas:");
  lines.forEach((line, i) => {
    const [user,,] = line.split(":");
    const pkg = packageNames[i];
    if (pkg) {
      console.log(`- ${user} en ${pkg}`);
    }
  });
}


MultiRejoinTool.prototype.logoutAllCookies = async function(rl) {
  // Detectar paquetes Roblox instalados
  const packages = Utils.detectAllRobloxPackages();
  const packageNames = Object.keys(packages);
  if (packageNames.length === 0) {
  ...
  console.log("\n🚪 Cerrando sesión (borrando cookies) en todas las apps Roblox...");
  packageNames.forEach(pkg => {
    try {
      execSync(`rm -f /data/data/${pkg}/app_webview/Default/Cookies`);
      console.log(`✅ [${pkg}] Cookie borrada.`);
    } catch (e) {
      console.log(`❌ Error al borrar cookie en ${pkg}: ${e.message}`);
    }
  });
...
╚══════════════════════════════════════╝`);
      ...
      console.log("7. 🍪 Injectar cookies desde archivo");
console.log("8. 🚪 Log out (borrar cookies)");

// Después del switch/case de opciones del menú principal:
if (choice.trim() === "7") {
  await this.injectCookiesFromFile(rl);
  rl.close();
} else if (choice.trim() === "8") {
  await this.logoutAllCookies(rl);
  rl.close();
}
